...
public class BrokerController {
    ...
    /**
     * 当管理器全部加载完毕后，控制器将开始进入下一步初始化，初始化多个线程池，
     * 包括sendMessageExecutor、pullMessageExecutor、adminBrokerExecutor、clientManagerExecutor,
     * 分别用于发送消息执行器、拉取消息执行器、broker管理器执行器、客户端管理执行器，
     * 这些执行器回放如线程池中处理, 来做并发执行。
     */
    public boolean initialize() throws CloneNotSupportedException {
        ...
        // 加载topicConfigManager、consumerOffsetManager、subscriptionGroupManager、consumerFilterManager
        // 将加载结果成功与否存储在result中
        if (result) {
            try {
                // 用于broker层的消息落地存储
